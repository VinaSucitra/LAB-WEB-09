    ...
    if ($project_manager_check && $project_manager_check['manager_id'] == $manager_id) {
        if ($assigned_to === NULL) {
            $stmt = $conn->prepare("
                ...
                VALUES (?, ?, ?, NULL, ?)
            ");
            $stmt->bind_param("iss", $project_id, $nama_tugas, $deskripsi, $status);
        } else {
            $stmt = $conn->prepare("
                INSERT INTO tasks (project_id, nama_tugas, deskripsi, assigned_to, status)
                VALUES (?, ?, ?, ?, ?)
        ...
        if ($stmt->execute()) {
            setFlashMessage('success', "Tugas " . htmlspecialchars($nama_tugas) . " berhasil ditambahkan!");
        } else {
...
<body class="text-pink-50 min-h-screen">
    ...
    <header class="bg-[#2a0e30] p-4 flex justify-between items-center shadow-md sticky top-0 z-10">
        <h1 class="text-2xl font-bold text-pink-400">VS Project Manager</h1>
    ...
    <main class="p-6 md:p-10 space-y-10">
        ...
        <section class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="card p-6 rounded-xl shadow-2xl">
                <h2 class="text-xl font-semibold text-pink-400 mb-4 border-b pb-2 border-pink-700">Buat Proyek Baru (C)</h2>
            ...
            <!-- FORM TAMBAH TUGAS -->
            <div class="card p-6 rounded-xl shadow-2xl">
                <h2 class="text-xl font-semibold text-pink-400 mb-4 border-b pb-2 border-pink-700">Tambahkan Tugas Baru (C)</h2>
